...
const char *relocate_path(const char *path, char *const buffer, const size_t size) {
    ALOGE("relocate_path");
    ALOGE(path);
    const char *result = "-------------------thefuck:relocate_path";
    return result;
}
...
HOOK_DEF(int, faccessat, int dirfd, const char *pathname, int mode, int flags) {
    char temp[PATH_MAX];
    ALOGE("IoHook: faccessat : %s", pathname);
...
HOOK_DEF(int, __statfs64, const char *pathname, size_t size, struct statfs *stat) {
    ...
    return orig___statfs64(pathname,size,stat);
}

// int lstat(const char *path, struct stat *buf);
HOOK_DEF(int, lstat, const char *pathname, struct stat *buf) {
    char temp[PATH_MAX];
    ALOGE("IoHook: lstat : %s", pathname);
